...
根据 2026-06-13 首轮体验反馈，当前底部主导航固定为：

1. **概览**：今日任务、近期日历事件、快捷入口和摘要。
2. **日历**：完整月历、日期事件列表和新增日历事件。
3. **空间**：任务、内容库、自定义记录、统计和 AI 助手。
4. **设置**：集成、安全、备份、主题和数据管理。

笔记、日记和书签不再作为三个并列入口，统一收敛到“内容库”。首版先统一
界面、搜索和编辑入口，底层继续通过适配器保留各自 repository 和存储方式，
避免破坏外部 Markdown、备份和已有数据。

“日历事件”指 Android Calendar Provider 数据；“自定义记录”指 tracking
模块中的模板化本地记录。两者在日期详情中可以聚合展示，但保持独立存储和
权限边界。

AI 助手使用全局悬浮入口或顶部入口，不单独占据底部导航。

详细修订和实施顺序见 `docs/PRODUCT_RESTRUCTURE_PLAN.md`。
