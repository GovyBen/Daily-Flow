...
| DF-404 | Existing My Brain AlarmManager receiver/boot flow plus the DF-402 Track & Graph-inspired fallback design | `core/alarm/.../ReminderUseCases.kt`, reminder scheduler/resolver contracts, `core/notification/.../ReminderReceiver.kt`, reminder fallback and shared restore scheduler, extended `AlarmSchedulerImpl` | Adapted/new Daily Flow integration 2026-06-15; idempotent state reconciliation, relative target resolution, stale-trigger protection, app/boot/time/permission restore and single-delivery terminal states |
