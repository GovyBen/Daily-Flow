...
- 完成记录（2026-06-13）：新增八类 sealed 输入值、统一验证结果和纯 Kotlin
  `TrackerValueValidator`。表驱动测试覆盖 required、活动选项、整数、范围、
  浮点步长容差、时长上限、有限数、精度和文本限制。
...
- 实现：
  ...
  - 用户删除或停用后不被下次启动恢复。
- 验收：已有用户升级不重复插入。
- 完成记录（2026-06-13）：新增健身、心情和习惯次数三套内置模板，模板、
  tracker、字段和选项均使用固定稳定 ID。应用通过 DataStore v1 标记只执行一次
  初始化；事务中断后按固定 ID 幂等重试，已停用模板不会被重新激活。雷电 Android 9
  实例验证首次启动写入 3 个模板、8 个 tracker、8 个字段和 7 个选项，第二次启动
  模板数保持为 3；tracking 12 个仪器测试、app/tracking JVM 测试和 lint 通过。
...
- 完成记录（2026-06-13）：
  新增记录空间入口和 Compose 模板列表，支持创建、复制、同分组排序、
  固定、停用确认，并展示字段数量和最后记录时间；列表由 Room Flow 驱动，
  创建对话框草稿可跨配置变化恢复。主数据库升级至 v7，增加模板固定状态和
  `MIGRATION_6_7`，repository 聚合 session 最后记录时间。雷电 Android 9
  上 tracking 23 个仪器测试和数据库 4 个迁移测试通过；保留旧数据覆盖安装
  后成功完成 v6 到 v7 升级，并验证三套默认模板、菜单和创建入口。debug
  构建、受影响模块编译及 lint 通过。
...
- 完成记录（2026-06-13）：
  新增创建/编辑导航和 Compose 模板编辑器，支持名称、描述、短图标、颜色、
  八类字段、必填与单位、类型特定范围/步进配置、选项增删排序、字段增删排序
  及动态输入预览。草稿通过序列化字符串和 `rememberSaveable` 跨配置变化恢复，
  领域校验统一覆盖名称、选项、重复标签和配置合法性。repository 在事务中
  停用被移除的 tracker/option；有数据点的 tracker 在 UI 和持久化边界均禁止
  改变类型，历史记录继续保留。雷电 Android 9 上 28 个 tracking 仪器测试
  全部通过，并完成已有模板编辑保存、新建入口、空名称校验和完整滚动预览
  冒烟；全工程 `testDebugUnitTest`、App lint、debug 构建及数据库测试编译通过。
...
- 完成记录（2026-06-13）：
  新增模板级快速记录导航、ViewModel 和单页 Compose 表单，一页展示全部动态
  字段，并支持修改发生日期/时间、填写会话备注及应用默认、最近、常用建议值。
  保存前统一执行领域校验，原子单飞状态阻止重复提交；Room 事务返回真实 session
  ID 后展示模板、发生时间和会话 ID，可直接继续记录或完成返回。模板卡片主点击
  进入快速记录，编辑入口移至操作菜单。JVM 测试覆盖命令时区偏移、备注清理和
  必填校验；雷电 Android 9 上 30 个 tracking 仪器测试通过，真实“健身”三字段
  模板以选择部位、保存两次操作完成记录，列表最后记录时间同步更新。App debug
  构建通过，运行日志无崩溃、ANR 或数据库异常。
...
- 完成记录（2026-06-13）：
  新增全局和模板级历史入口、typed navigation、模板下拉与本地时区日期筛选，
  按会话展示发生时间、模板全部当前字段、会话备注和已归档字段。历史数据点可
  反解为八类动态输入，编辑页支持修改发生时间、字段值和会话备注，并通过
  `UpdateRecordSessionUseCase` 整会话事务更新。repository 更新时保留已从模板
  移除的归档数据点，并同步新的发生时间，避免历史字段丢失。删除先确认，随后
  UI 立即隐藏会话并提供 Snackbar 短时撤销；撤销窗口结束后才调用整会话级联
  删除事务。JVM 测试覆盖夏令时 23 小时日界、八类反解、时区命令和归档展示；
  雷电 Android 9 上 34 个 tracking 仪器测试通过。真实 app 完成历史筛选、字段
  编辑保存、删除确认、撤销恢复和永久删除路径验证；全工程 JVM 测试、tracking/
  App lint 与 debug 构建通过，运行日志无崩溃、ANR 或数据库异常。
...
- 完成记录（2026-06-13）：
  空间卡片名称调整为“自定义记录”，继续保留概览、日历、空间、设置四栏主导航，
  任务入口和内容库路径不变。概览顶部新增自定义记录卡片，按固定、最近记录时间和
  模板顺序展示最多四个快捷模板，并按设备本地日界实时汇总今日会话总数和模板计数；
  卡片可进入 typed quick-record 与全局历史路由。JVM 测试覆盖快捷模板排序和摘要
  计数，雷电 Android 9 上 35 个 tracking 仪器测试通过。真实 app 验证空间入口、
  今日摘要、快捷记录和历史页返回栈；快捷记录页置后台并杀进程后可恢复原页面，
  运行日志无主线程崩溃。全工程 JVM 测试、lint 与 debug 构建通过。
...
- 完成记录（2026-06-14）：
  月视图日期格使用独立的事件圆点和本地记录条形指示，日期详情按“日历事件”和
  “当日记录”分组；新增入口先明确选择 Calendar Provider 事件或本地 tracking，
  本地路径再选择模板并把所选日期带入快速记录页。新增跨模板轻量记录查询，模板
  停用后仍保留历史名称和颜色；日历权限缺失或 Provider 查询失败时强制保留月视图、
  tracking 查询和新增能力，仅事件分组显示授权提示。修复月缓存跨年键冲突，并按
  本地日期边界处理 DST、跨日事件和 Android 全天事件 UTC 日期语义。JVM 测试覆盖
  DST 跨午夜、全天事件、无权限不查询 Provider 和设备时区记录归日；雷电 Android 9
  上 tracking 仪器测试 36/36。模拟器分别验证授权与拒绝路径、目标选择、2026-06-15
  日期传递和真实“健身”记录保存，返回日历后日期指示与详情即时刷新，日志无崩溃。
  全工程 JVM 测试、lint、app debug 与 app instrumentation APK 组装通过。
...
- 完成记录（2026-06-14）：
  新增统计领域结果模型、只暴露范围读取的 `TrackingAnalyticsRepository` 及 Room
  实现，并提供日摘要、周序列、月序列、选项分布、当前连续天数和最长连续天数六个
  用例。聚合引擎保持纯 Kotlin，复用 DF-301 分箱和 DF-302 样本适配；序列补齐空箱，
  停用 tracker 仍保留历史名称和单位，布尔连续天数只统计 true，自定义日界线和
  设备时区均参与归日。JVM 测试覆盖空数据、单点、纽约夏令时 23 小时日、跨月/跨年、
  停用 tracker、选项分布、布尔值和 05:00 日界线；Room 仪器测试覆盖半开区间、
  全历史范围和缺失 tracker。`tracking` JVM 测试、lint、app debug 构建通过，雷电
  Android 9 上 38/38 仪器测试通过，最新 APK 安装启动无崩溃或 Koin 注入异常。
