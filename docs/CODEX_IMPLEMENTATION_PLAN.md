...
#### DF-501 建立 DeepSeek Provider 预设

- [ ] 状态
- 前置：DF-004
- 复用：
  - My Brain `AiProvider.OpenAI`
  - custom base URL
  - Ktor 和 Koog 配置
- 新增：
  - DeepSeek 预设名称、默认 HTTPS endpoint 和可编辑 model。
  - 连接测试。
- 要求：不硬编码 API Key；错误区分认证、限流、网络和 tool calling 不兼容。
- 验收：使用 fake server 完成 request/response 测试。
...
基线稳定后，第二轮从 `DF-101` 开始。每轮最多处理一个阶段中的 2 至 4 个相邻任务，并在可安装里程碑处生成 APK 进行设备验证。
