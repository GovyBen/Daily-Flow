...
4. 修改未来截止时间会用原 `alarmId` upsert，并以相同 `requestCode` 更新
   `PendingIntent`，所以单个任务的提醒不会因普通修改而叠加。
5. 移除截止时间、首次完成任务或删除任务会先取消 AlarmManager，再删除
   `alarms` 行。取消 intent 使用同一 receiver 和 `requestCode`；extra 不参与
   `PendingIntent` identity，因此能够匹配原提醒。
...
- 没有监听 `TIME_CHANGED`、`TIMEZONE_CHANGED` 或 `DATE_CHANGED`，也没有统一重算。

## Exact Alarm 和通知权限

- Manifest 声明 `SCHEDULE_EXACT_ALARM`。Android 12 及以上通过
  `AlarmManager.canScheduleExactAlarms()` 检查；更低版本视为可用。
- 可用时调用 `AlarmManagerCompat.setExactAndAllowWhileIdle()`。
- 不可用时 `UpsertAlarmUseCase` 返回 `null`，任务仍保存截止时间但不保存
  `alarmId`，调用方以失败结果显示授权提示；当前没有非精确或 WorkManager 降级。
- 新增任务在保存后显示授权提示；任务详情在启用截止时间时预先提示。
...
- Manifest 声明 `RECEIVE_BOOT_COMPLETED`，`BootBroadcastReceiver` 仅监听
  `BOOT_COMPLETED`。
- receiver 使用 `goAsync()`，读取 `alarms` 全表并逐条重新调用 scheduler。
- 当前不处理 `LOCKED_BOOT_COMPLETED`、`MY_PACKAGE_REPLACED`、时间或时区变化。
- 恢复不检查提醒是否已过期，也不捕获单条调度失败；一条异常可中止后续恢复。
...
- 移除截止时间和完成任务会取消并删除提醒。
- exact alarm 权限不可用时保存任务但返回“未调度”结果。

运行：

```powershell
.\gradlew.bat :tasks:domain:test --no-problems-report
```
...
- exact alarm 不可用时没有降级路径，这是 DF-402 的首要修复目标。
- alarm 持久化先于系统调度；权限在检查后被撤销或 scheduler 抛错时可能留下孤立行。
- 已触发提醒只删除 alarm 行，不清空 `Task.alarmId`。
- 开机恢复会重排过期提醒，且缺少逐条隔离、幂等核对和应用升级恢复。
- `AlarmReceiver` 没有用 `try/finally` 保证所有异常路径都结束 `PendingResult`。
